...
function showFrames(elem, video, event, true_event, aux_events,
    ...
    if (frame_div.length == 0) {
        ...
        function addFrame(i, context_type) {
            // MODIFICATION: Détecter si c'est la frame de l'événement
            let is_event_frame = (event && event.frame == i) || (true_event && true_event.frame == i);
            
            let rgb = $('<a>').prop({
                target: '_blank', href: `full_res/${video.replaceAll('/', '=')}/${i}`
            }).append($('<img>').prop('src', `/rgb/${video.replaceAll('/', '=')}/${i}`));
            // SUPPRIMÉ: let flow = ... (pas besoin, on n'affiche pas les images flow)
            
            // MODIFICATION: Créer le frame-block avec classe conditionnelle
            let frameBlock = $('<div>').addClass('frame-block');
            if (is_event_frame) {
                frameBlock.addClass('event-frame');
            }
            
            frameBlock.append(
                $('<div>').addClass('frame-info').append(
                    // Numéro de frame EN HAUT
                    $('<span>').text(i),
                    // Images (au milieu) - SEULEMENT RGB, PAS FLOW
                    context_type == 'sparse' ?
                        rgb.addClass('context-frame') : rgb,
                    // SUPPRIMÉ: images flow (carrés noirs)
                    // context_type == 'sparse' ?
