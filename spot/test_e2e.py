...
def main(model_dir, frame_dir, split, no_overlap, save, save_as, dataset):
    ...
    # MODIFICATION ICI : Forcer device='cpu'
    model = E2EModel(
        len(classes) + 1, config['feature_arch'], config['temporal_arch'],
        clip_len=config['clip_len'], modality=config['modality'],
        device='cpu',  # ← CHANGEMENT ICI
        multi_gpu=False)  # ← ET ICI (au lieu de config['gpu_parallel'])
    
    # Charger le checkpoint avec map_location
    model.load(torch.load(
        os.path.join(model_dir, 'checkpoint_{:03d}.pt'.format(best_epoch)),
        map_location='cpu'))  # ← CHANGEMENT ICI
